...
2. **Decisions.** If the branch came from a `plan:<slug>`: read the comments on its todos (`todo_list` for the tag, completed included; `comment_list` on each) and append a `## Deviations` section to `docs/decisions/YYYY-MM-DD-<slug>.md` — same basename as the `docs/plans/*-<slug>.md` file in this branch, never today's date; append only if the file has no `## Deviations` section yet — one bullet per place the build, or this review's fixes, left the plan: what changed, why, commit hash. Write `None.` when there were none. If build skipped the file (the plan named no design scratchpad), create it with a `# <slug>` H1 only when there is a deviation to record.
