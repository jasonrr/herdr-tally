...
You are the controller: dispatch, verify, adjudicate. Subagents implement. Work on a branch or worktree, never main — in herdr (`HERDR_ENV=1`), `herdr worktree create --cwd <repo-root> --branch feat/<slug> --no-focus` and run the whole loop from that worktree; otherwise EnterWorktree.
...
When every task is complete, run /tally:review-branch.
