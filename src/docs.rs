...
// read-only Docs tab: it loads a per-user list of doc directories and
...
/// Browsed when no doc-paths config exists, relative to the project root.
...
/// One markdown file surfaced in the Docs tab. Port of Go's `Doc`.
#[derive(Debug, Clone)]
pub struct Doc {
...
/// Resolves the doc-paths file location from explicit env values. Kept pure and
...
/// The doc-paths file location, reading the env at the public boundary.
...
/// Reads and parses the newline-delimited doc-paths file at `path` (# comments
/// and blank lines ignored), falling back to `defaults()` when the file is
/// absent or yields nothing. Split out from `load_doc_paths` so tests can pass a
/// path directly instead of mutating env.
fn load_doc_paths_from(path: Option<&Path>) -> Vec<String> {
...
/// Reads the newline-delimited doc-paths config and returns the configured
/// relative dirs, falling back to `defaultPaths` when absent or empty. Port of
/// Go's `LoadDocPaths`.
pub fn load_doc_paths() -> Vec<String> {
    load_doc_paths_from(config_file().as_deref())
}
...
/// Go's `List`.
pub fn list(root: &Path, paths: &[String]) -> Vec<Doc> {
...
// dereference).
fn walk(dir: &Path, root: &Path, seen: &mut HashSet<PathBuf>, out: &mut Vec<Doc>) {
...
mod tests {
    ...
    fn load_doc_paths_parses_file() {
        let dir = TempDir::new();
        let file = dir.path().join("doc-paths");
        fs::write(&file, "# my docs\ndocs/plans\n\n  docs/notes  \n").unwrap();
        let got = load_doc_paths_from(Some(&file));
    ...
    fn load_doc_paths_defaults_when_absent() {
        let dir = TempDir::new();
        let file = dir.path().join("doc-paths"); // dir exists, no doc-paths file
        let got = load_doc_paths_from(Some(&file));
        assert_eq!(
            got,
            vec![
    ...
    fn config_file_prefers_plugin_config_dir() {
        let got = config_file_from(
            Some("/plug".to_string()),
            Some("/x".to_string()),
            Some("/h".to_string()),
        );
        assert_eq!(got, Some(PathBuf::from("/plug/doc-paths")));
    }
    ...
    fn list_collects_configured_markdown_sorted_by_mtime() {
        ...
        assert_eq!(got.len(), 3, "want 3 docs, got {got:?}");
